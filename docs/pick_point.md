...
![pick point debug](img/pick_point_debug.png)

> 위 이미지는 실제 hand-eye 가중치(`upright-cup` 클래스)로 컵 정지영상에 노드와
> **동일한 검출 + 내접원 + debug 그리기** 로직을 적용한 결과다. 우상단 컵은 옆면이
> 아래로 잡혀 회색(centroid)이 내려가 있지만 빨강(pick)은 윗면 원 쪽으로
> 올라온 것을 볼 수 있다. (예시 영상엔 다른 노드의 주석이 겹쳐 그려져 있고,
> depth/FK/로봇 없이 2D 검출만으로 산출되는 debug 부분만 재현한 것이다 —
> 라이브 카메라+로봇 end-to-end 검증은 실제 리그에서 별도 필요.)
